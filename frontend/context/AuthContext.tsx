  ...
  const fetchProfile = async (userId: string) => {
    try {
      ...
      if (error && error.code === 'PGRST116') {
        ...
        // onboarding_completed is false so the user goes through the full flow:
        // Welcome → Register → OTP → Onboarding → Dashboard
        ...
        const { data: newProfile, error: createError } = await supabase
          ...
          .insert({
            ...
            onboarding_completed: false, // New users must complete onboarding
        ...
        if (retryData && !retryError) {
          ...
          // onboarding_completed defaults to true here to protect returning
          // users from being sent back through onboarding during a DB outage.
          const { data: userData } = await supabase.auth.getUser();
          ...
          setProfile({
            ...
            xp: 0,
            power_level: 0,
            onboarding_completed: true, // Safe fallback during DB outage
            level_title: 'Beginner',
