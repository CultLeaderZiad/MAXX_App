...
export function TrialBanner() {
  ...
  useEffect(() => {
    ...
    const startTime = profile.trial_start ? new Date(profile.trial_start) : new Date(profile.created_at || new Date());
  ...
  return (
    <View style={[styles.container, { backgroundColor: 'rgba(200, 169, 110, 0.1)', borderColor: theme.gold }]}>
      ...
      <Text style={[styles.text, { color: theme.gold }]}>
        <Text style={{ fontFamily: FONTS.bold }}>{daysLeft} days</Text> left in your {profile.plan ? profile.plan.toUpperCase() : 'Free'} Trial
      </Text>
    </View>
  );
}
