...
export function EmptyState({ icon = 'inbox', title, subtitle, actionLabel, onAction }: Props) {
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={[styles.title, { color: theme.textPrimary, fontFamily: FONTS.cinzelBold }]}>{title}</Text>
      {subtitle && (
        <Text style={[styles.subtitle, { color: theme.textMuted, fontFamily: FONTS.regular }]}>{subtitle}</Text>
      )}
      {actionLabel && onAction && (
        <TouchableOpacity onPress={onAction} style={[styles.actionBtn, { backgroundColor: theme.gold }]}>
