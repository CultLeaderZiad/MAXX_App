...
export default function EmergencyScreen() {
  ...
  const toggleTask = (id: number) => {
    setTasks((prev: any[]) =>
      prev.map((t: any) => (t.id === id ? { ...t, checked: !t.checked } : t)),
    );
  };
      ...
      {sosActive ? (
        <View style={[styles.sosContainer, { backgroundColor: "#000" }]}>
          <Text
            style={[
              styles.sosTitle,
              { color: "#E74C3C", fontFamily: FONTS.cinzelBold },
            ]}
          >
            DONT GIVE UP
          </Text>
          <Text style={[styles.sosSub, { color: "#FFF" }]}>
            The pain of discipline is better than the pain of regret.
          </Text>

          <Animated.View
            style={{ transform: [{ scale: sosPulse }], marginTop: 60 }}
          >
            <TouchableOpacity onPress={startSOS} style={styles.sosCircle}>
              <Text
                style={{ color: "#FFF", fontFamily: FONTS.bold, fontSize: 32 }}
              >
                {sosCountdown === 30 ? "HELP" : sosCountdown}
              </Text>
            </TouchableOpacity>
          </Animated.View>
        ...
        <View style={styles.content}>
          ...
          <View style={styles.actionSection}>
            ...
            >
              STEP 2 — PHYSICAL ACTION (CHECK 1+)
            </Text>
            {tasks.map((task) => (
              <TouchableOpacity
                key={task.id}
                ...
                style={[
                  styles.taskBtn,
                  {
                    ...
                    borderColor: task.checked ? theme.gold : theme.border,
                  },
                ]}
              >
                <View
          ...
          <View style={styles.footer}>
            ...
            >
              Must complete 1 physical task to exit
            </Text>
          </View>
        </View>
      )}
