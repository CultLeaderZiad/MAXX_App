...
import { View, Text, StyleSheet, TouchableOpacity, ScrollView, Switch, Alert, TextInput, Modal } from 'react-native';
...
export default function SettingsScreen() {
  ...
  const handleDeleteAccount = () => {
    Alert.alert('Delete Account', 'This action is permanent. Contact support@maxx.app to delete your account.', [
      { text: 'Cancel', style: 'cancel' },
      { text: 'Sign Out', style: 'destructive', onPress: async () => {
        await signOut();
        router.replace('/');
  ...
  const planLabel: Record<string, string> = { free_trial: 'Free Trial', grind: 'Grind', alpha: 'Alpha', sigma: 'Sigma' };
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: theme.bgPrimary }]} testID="settings-screen">
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        <Section title="ACCOUNT">
          ...
          <Item label="Delete Account" value="Contact support" color="#E74C3C" onPress={handleDeleteAccount} icon={null} />
        </Section>

        <Section title="SECURITY">
          <ToggleItem label="Face ID / Touch ID" value={false} onToggle={() => Alert.alert('Biometrics', 'Configure in your device settings.')} />
          <Item label="Active Sessions" value="Manage in Supabase" onPress={() => {}} />
        </Section>
        ...
        <Section title="SUBSCRIPTION">
          <Item label="Current Plan" value={currentPlanLabel} onPress={() => router.push('/plans')} />
          <Item label="Cancel Subscription" color={theme.textMuted} onPress={() => Alert.alert('Cancel', 'Contact support@maxx.app to cancel.')} icon={null} />
        </Section>

        <Section title="SUPPORT">
          <Item label="Contact Support" onPress={() => router.push('/support' as any)} />
          <Item label="Supplements" onPress={() => router.push('/supplements' as any)} />
        </Section>

        <View style={styles.footer}>
          <Button title="SIGN OUT" onPress={handleSignOut} variant="danger" />
          <Text style={[styles.version, { color: theme.textMuted, fontFamily: FONTS.regular }]}>MAXX v1.0.0 — {user?.email || 'Member'}</Text>
        </View>
      </ScrollView>
      ...
      <Modal visible={showPasswordModal} transparent animationType="slide">
        <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.8)', justifyContent: 'flex-end' }}>
          <View style={{ backgroundColor: theme.bgSurface, borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 24 }}>
            <Text style={{ color: theme.textPrimary, fontFamily: FONTS.cinzelBold, fontSize: 18, marginBottom: 20 }}>Change Password</Text>
            <Text style={{ color: theme.textMuted, fontSize: 12, marginBottom: 6 }}>NEW PASSWORD</Text>
            <TextInput
              style={{ backgroundColor: theme.bgElevated, color: theme.textPrimary, borderRadius: 10, padding: 14, marginBottom: 12, borderWidth: 1, borderColor: theme.border }}
            ...
            />
            <Text style={{ color: theme.textMuted, fontSize: 12, marginBottom: 6 }}>CONFIRM PASSWORD</Text>
            <TextInput
            ...
            <View style={{ flexDirection: 'row', gap: 12 }}>
              <TouchableOpacity onPress={() => setShowPasswordModal(false)} style={{ flex: 1, padding: 14, borderRadius: 10, backgroundColor: theme.bgElevated, alignItems: 'center' }}>
                <Text style={{ color: theme.textMuted, fontFamily: FONTS.semiBold }}>Cancel</Text>
              </TouchableOpacity>
              <TouchableOpacity onPress={handleChangePassword} style={{ flex: 2, padding: 14, borderRadius: 10, backgroundColor: theme.gold, alignItems: 'center' }} disabled={changingPw}>
                <Text style={{ color: '#0A0A0A', fontFamily: FONTS.bold }}>{changingPw ? 'Saving...' : 'CHANGE PASSWORD'}</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
